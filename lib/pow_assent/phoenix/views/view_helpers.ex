defmodule PowAssent.Phoenix.ViewHelpers do
  ...
  defp oauth_signin_link(conn, provider) do
    msg  = Controller.messages(conn, Messages).login_with_provider(conn, Naming.humanize(provider))
  ...
  defp oauth_remove_link(conn, provider) do
    msg  = Controller.messages(conn, Messages).remove_provider_authentication(conn, Naming.humanize(provider))
